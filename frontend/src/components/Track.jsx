...
const Track = ({ video, onPlay, currentTrack, isPlaying, handlePlayPause, toggleFavorite, isFavorite }) => {
  ...
  return (
    <div className="flex items-center justify-between p-3 rounded-lg hover:bg-purple-900/20 transition-colors group">
      {/* Thumbnail and Play Button */}
      <div className="relative w-10 h-10 sm:w-12 sm:h-12 flex-shrink-0" onClick={() => togglePlay(video.id.videoId)}>
        {/* Thumbnail */}
        <img
        ...
        <button
          ...
          className={`absolute inset-0 flex items-center justify-center w-full h-full bg-black/50 rounded-md opacity-${currentTrack === video.id.videoId && isPlaying ? '100' : '0'
